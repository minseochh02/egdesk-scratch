...
const directory = '창고별재고-january/february';

console.log('='.repeat(80));
console.log('Excel Inventory Data Import Script - FEBRUARY 2026');
console.log('='.repeat(80));
  ...
  try {
    ...
    for (const cell of row0) {
      if (cell && typeof cell === 'string') {
        // Match pattern: "회사명 : (주)영일오엔씨 / 2026/02/01"
    ...
    if (!importDate) {
      console.log(`  ⚠ Warning: Could not extract date from row 0, using filename date`);
      // Try to extract date from filename (format: MMDD.xlsx)
      const filenameMatch = filename.match(/(\d{2})(\d{2})\.xlsx/);
      if (filenameMatch) {
        const [, month, day] = filenameMatch;
        importDate = `2026-${month}-${day}`;
      } else {
...
// Print summary
console.log('='.repeat(80));
console.log('IMPORT SUMMARY - FEBRUARY 2026');
console.log('='.repeat(80));
...
const statsFeb = dbVerify.prepare(`
  SELECT
    COUNT(*) as total_rows,
    COUNT(DISTINCT imported_at) as unique_dates,
    MIN(imported_at) as earliest_date,
    MAX(imported_at) as latest_date
  FROM inventory
  WHERE imported_at >= '2026-02-01' AND imported_at <= '2026-02-28'
`).get();
dbVerify.close();
...
console.log(`  Total rows: ${statsFeb.total_rows}`);
console.log(`  Unique dates: ${statsFeb.unique_dates}`);
console.log(`  Date range: ${statsFeb.earliest_date} to ${statsFeb.latest_date}`);
console.log();
