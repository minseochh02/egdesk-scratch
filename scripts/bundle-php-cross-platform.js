...
async function bundleSystemPHP(platformInfo, systemKey) {
  ...
  const bundleDir = path.join(__dirname, '..', 'php-bundle', platformInfo.bundleDir);
  ...
  try {
    if (platformInfo.phpPath && fs.existsSync(platformInfo.phpPath)) {
      ...
      const version = execSync(`${platformInfo.phpPath} --version`, { encoding: 'utf8' });
      ...
      if (os.platform() === 'darwin') {
        const deps = execSync(`otool -L ${platformInfo.phpPath}`, { encoding: 'utf8' });
        const libLines = deps.split('\n').filter(line => 
          line.includes('/opt/homebrew/') || 
          line.includes('/usr/local/')
        );
...
`;
    ...
    } else if (os.platform() === 'win32' || platformInfo.bundleDir.startsWith('windows/')) {
...
`;
    ...
    console.error(`❌ Failed to bundle PHP for ${platformInfo.name}:`, error.message);
    throw error;
  }
}
  ...
  for (const [systemKey, platformInfo] of Object.entries(PLATFORMS)) {
    try {
      ...
      if (systemKey === currentSystem) {
        ...
        results[systemKey] = path.join(__dirname, '..', 'php-bundle', platformInfo.bundleDir);
      } else {
...
module.exports = { bundleAllPlatforms, bundleSystemPHP, detectSystem, PLATFORMS };
