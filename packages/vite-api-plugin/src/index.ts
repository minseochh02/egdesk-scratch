  ...
  return {
    ...
    async configureServer(server: ViteDevServer) {
      ...
      server.middlewares.use(async (req, res, next) => {
        ...
        // AI Center MCP proxy (workflows, entities, relations, tags)
        if (url.includes('__ai_center_proxy')) {
          log(`Proxying AI center request: ${url}`);
          ...
          req.on('end', async () => {
            try {
              ...
              const response = await fetch(`${actualApiUrl}/ai-center/tools/call`, {
                method: 'POST',
                headers,
                body
              });
              ...
              console.error('[vite-api-plugin] AI center proxy error:', error);
              res.statusCode = 500;
              res.setHeader('Content-Type', 'application/json');
              res.end(JSON.stringify({
                error: 'Proxy error',
