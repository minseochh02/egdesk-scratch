...
// 계정 ID 목록 조회
function listKakaoAccountIds(cfg: OpenClawConfig): string[] {
  const kakaoConfig = (cfg.channels as any)?.kakao as KakaoConfig | undefined;
...
// 콜백 대기 응답
export function createCallbackWaitResponse(waitMessage?: string): KakaoSkillResponse {
  return {
    version: "2.0",
    useCallback: true,
    ...(waitMessage ? {
...
// 콜백 URL로 응답 전송
async function sendCallbackResponse(
  callbackUrl: string,
  ...
  try {
    ...
    const result = await res.json() as { taskId: string; status: string; message: string };
    return {
...
export const kakaoPlugin: ChannelPlugin<ResolvedKakaoAccount> = {
  ...
  configSchema: { schema: KakaoConfigSchema } as any,
  config: {
    listAccountIds: (cfg: OpenClawConfig) => listKakaoAccountIds(cfg),
    resolveAccount: (cfg: OpenClawConfig, accountId?: string | null) => resolveKakaoAccount(cfg, accountId),
    ...
    isConfigured: (account: ResolvedKakaoAccount) => Boolean(account.webhookPath),
  ...
  security: {
    resolveDmPolicy: ({ cfg, accountId, account }: any) => {
      ...
      const useAccountPath = Boolean(
        ((cfg.channels as any)?.kakao as KakaoConfig | undefined)?.accounts?.[resolvedAccountId]
      );
      ...
      return {
        ...
        normalizeEntry: (raw: string) => raw.replace(/^kakao:(?:user:)?/i, ""),
      };
    },
    collectWarnings: () => [],
  },
  messaging: {
    ...
    targetResolver: {
      looksLikeId: (id: string | null | undefined) => {
  ...
  setup: {
    ...
    validateInput: ({ input }: any) => {
      const typedInput = input as { webhookPath?: string };
  ...
  status: {
    ...
    buildChannelSummary: async ({ account, snapshot }: any) => {
      return {
  ...
  gateway: {
    startAccount: async (ctx: any) => {
      const account = ctx.account as ResolvedKakaoAccount;
...
// AI 응답 처리 및 콜백 전송
export async function processAndSendCallback(
  ctxPayload: {
    From: string;
    Body: string;
    Channel: string;
  ...
  runtime: ReturnType<typeof getKakaoRuntime>
) {
  try {
    let responseText = "";
    ...
    await sendCallbackResponse(
      callbackUrl,
      createSimpleTextResponse("죄송합니다. 응답 생성 중 오류가 발생했습니다.")
    );
  }
}
