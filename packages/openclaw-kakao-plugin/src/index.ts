...
export default defineChannelPluginEntry({
  ...
  registerFull(api: OpenClawPluginApi) {
    ...
    const kakaoSkillHandler = async (req: any, res: any) => {
      try {
        ...
        const body = JSON.parse(bodyText) as KakaoSkillRequest;
        const userKey =
        ...
        const account = resolveKakaoAccount(cfg as any, null);
        ...
        if (account.useCallback && callbackUrl) {
          ...
          processAndSendCallback(ctxPayload, callbackUrl, cfg, account, runtime).catch(
            (err: unknown) => {
