...
const RevertTest: React.FC = () => {
  ...
  const testFindBackups = async () => {
    ...
    try {
      ...
      const sampleFilePath =
        '/Users/minseocha/Desktop/projects/태화트랜스/Taehwa_demo/www/index.php';
      ...
      if (backups.length > 0) {
        ...
        backups.forEach((backup, index) => {
          addResult(
            `   ${index + 1}. ${backup.backupFilePath} (${backup.timestamp.toLocaleString()}) - ${backup.isValid ? 'Valid' : 'Invalid'}`,
          );
        });
      } else {
      ...
      addResult(
        `❌ Error testing backup discovery: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
    } finally {
      setLoading(false);
    }
  ...
  const testFindAllBackups = async () => {
    ...
    try {
      const projectRoot =
        '/Users/minseocha/Desktop/projects/태화트랜스/Taehwa_demo';
      ...
      addResult(
        `📊 Total: ${totalBackups} backup files across ${allBackups.size} original files`,
      );
    } catch (error) {
      addResult(
        `❌ Error testing project backup discovery: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
    } finally {
      setLoading(false);
    }
    ...
    try {
      ...
      if (backups.length > 0) {
        ...
        if (preview.success) {
          ...
          addResult(
            `   Current content: ${preview.currentContent?.length || 0} characters`,
          );
          addResult(
            `   Backup content: ${preview.backupContent?.length || 0} characters`,
          );
          if (preview.diff) {
            addResult(
              `   Changes: +${preview.diff.added} -${preview.diff.removed} ~${preview.diff.modified} lines`,
            );
          }
        } else {
      ...
      addResult(
        `❌ Error testing revert preview: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <ul>
          <li>
            <strong>Find Backups:</strong> Tests finding backup files for a
            specific file
          </li>
          <li>
            <strong>Find All Backups:</strong> Tests discovering all backup
            files in a project directory
          </li>
          <li>
            <strong>Test Preview:</strong> Tests generating a preview of what
            would change during revert
          </li>
        </ul>
        <p>
          <strong>Note:</strong> These tests use the example backup file path
          from your message. Adjust the file paths in the code if needed for
          your specific setup.
        </p>
      </div>
    </div>
