...
export class VoidMessageWeightingService {
   ...
   */
  static calculateReservedOutputSpace(
    contextWindow: number,
    customReservation?: number,
  ): number {
    return Math.max(
  ...
  static findLargestByWeight(
    ...
    for (let i = 0; i < messages.length; i += 1) {
      const m = messages[i];
      const w = this.calculateMessageWeight(
        m,
        messages,
        i,
        alreadyTrimmedIndexes,
      );
      if (w > largestWeight) {
        largestWeight = w;
        largestIndex = i;
  ...
  static fitMessagesToContextWindow(
    ...
    const reservedOutput = this.calculateReservedOutputSpace(
      contextWindow,
    ...
    while (remainingCharsToTrim > 0) {
      ...
      const trimIdx = this.findLargestByWeight(
        trimmedMessages,
        alreadyTrimmedIndexes,
      );
      ...
      if (numCharsWillTrim > remainingCharsToTrim) {
        ...
        m.content = `${m.content
          .slice(0, m.content.length - remainingCharsToTrim - '...'.length)
          .trim()}...`;
        break;
      }
