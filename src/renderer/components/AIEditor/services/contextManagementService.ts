...
class ContextGatheringService {
  ...
  private _getSnippetForRange(
    ...
    const adjustedStartLine =
      totalLines > this._MAX_SNIPPET_LINES
  ...
  private async _gatherNearbySnippets(
    ...
    const endLine = Math.min(
      pos.lineNumber + numLines,
      model.getLineCount ? model.getLineCount() : 1,
    );
  ...
  private async _gatherParentSnippets(
    ...
    const containerRange =
      container.kind === 'method' ? container.selectionRange : container.range;
    this._addSnippetIfNotOverlapping(model, containerRange, snippets, visited);

    const symbols = await this._getSymbolsNearRange(
      model,
      containerRange,
      numLines,
    );
    ...
    const containerPos = {
      lineNumber: containerRange.startLineNumber,
      column: containerRange.startColumn,
    };
    await this._gatherParentSnippets(
      model,
      containerPos,
  ...
  private async _getSymbolsNearRange(
    ...
    const centerLine = Math.floor(
      (range.startLineNumber + range.endLineNumber) / 2,
    );
  ...
  private async _getSymbolsInRange(model: any, range: any): Promise<any[]> {
    ...
    for (
      ...
      line <= range.endLineNumber && line <= lines.length;
      line++
    ) {
      ...
      for (const word of words) {
        if (word.length > 2) {
          // Only consider meaningful symbols
          symbols.push({
            name: word,
            detail: '',
            kind: 'variable',
