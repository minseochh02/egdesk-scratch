...
export const AIEditor: React.FC<AIEditorProps> = ({
  ...
  const [codespaceChatService, setCodespaceChatService] =
    useState<CodespaceChatService | null>(null);
...
🚨 REMEMBER: You are responsible for making the code changes. Provide search/replace operations for everything that needs to be modified. The user cannot edit code themselves.`,
  ...
  useEffect(() => {
    if (
      config.provider &&
      (!selectedKey || selectedKey.providerId !== config.provider)
    ) {
      setSelectedKey(null);
      setSelectedModel('');
    }
  ...
  const forceRefresh = async () => {
    try {
      if (projectContext?.currentProject?.path) {
        EnhancedAIEditorService.forceRefresh(
          projectContext.currentProject.path,
        );
  ...
  const handleRequestEdit = async () => {
    ...
    try {
      ...
      if (
        ...
        console.log(
          '🔍 Normalized file path from absolute to relative:',
          normalizedPath,
        );
      ...
      if (codespaceChatService && config.includeContext) {
        try {
          ...
          const enhancedMessage =
            await codespaceChatService.enhanceMessageWithContext(
              userInstruction,
              selectedKey,
              selectedModel,
            );
          ...
          if (
            enhancedMessage.codespaceContext?.primaryFiles?.length &&
            enhancedMessage.codespaceContext.primaryFiles.length > 0
          ) {
            hasEnhancedContext = true;

            // Use primary files (full content) instead of search snippets
            const primaryFileContexts =
              enhancedMessage.codespaceContext.primaryFiles
                .map((pf) => {
            ...
            if (
              ...
              enhancedMessage.codespaceContext.secondaryTechMeta.length > 0
            ) {
              secondaryContext = `\n\nSecondary/Technical References:\n${enhancedMessage.codespaceContext.secondaryTechMeta
                .map(
                  (meta) =>
                    `- ${meta.category.toUpperCase()}: ${meta.path}${meta.description ? ` — ${meta.description}` : ''}`,
                )
                .join('\n')}`;
            }
            ...
            console.log(
              '🔍 Enhanced instruction with PRIMARY files:',
              enhancedMessage.codespaceContext.primaryFiles.length,
              'files',
            );
          }
          ...
          console.warn(
            '🔍 Failed to enhance context, using basic context:',
            error,
          );
        }
      }
  ...
  const handleApplyEdits = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        alert(
          `Successfully applied ${aiResponse.edits.length} edit(s) to ${result.modifiedFiles.length} file(s)`,
        );
      } else {
      ...
      alert(
        `Error applying edits: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
    }
  };

  // Apply edits directly to files (Cursor-style)
  const applyEditsToFiles = async (
    edits: AIEdit[],
  ): Promise<{
    success: boolean;
          ...
          if (edit.type === 'create' && edit.filePath && edit.newText) {
            ...
            try {
              if (projectContext?.currentProject?.path) {
                ...
                targetPath = vector.suggestUniquePath(
                  projectContext.currentProject.path,
                  edit.filePath,
                );
              }
            ...
            // Delete file
            const result = await window.electron.fileSystem.deleteItem(
              edit.filePath,
            );
            if (result.success) {
            ...
            if (result.success) {
              ...
              errors.push(
                `Failed to modify ${currentFileData.path}: ${result.error}`,
              );
            }
          } else if (
            edit.type === 'replace' ||
            ...
            errors.push(
              `Invalid edit operation: ${edit.type} - missing required properties`,
            );
          }
        } catch (error) {
          errors.push(
            `Error processing edit: ${error instanceof Error ? error.message : 'Unknown error'}`,
          );
        }
      }
  ...
  const availableKeys = aiKeys.filter(
    (key) => key.providerId === config.provider,
  );
            ...
            <div className="conversation-actions">
              {onToggleEditing && (
                ...
                >
                  {isEditing ? (
                    <>
                      <FontAwesomeIcon icon={faGlobe} /> Show Server
                    </>
                  ) : (
                    <>
                      <FontAwesomeIcon icon={faEdit} /> Show Editor
                    </>
                  )}
                </button>
              )}
          ...
          {error && (
            <div className="message error-message">
              <div className="message-content">
                ...
                <button onClick={handleClearError} className="retry-btn">
                  Try Again
                </button>
              </div>
            </div>
              ...
              <div className="message-content">
                <div className="response-header">
                  <span className="response-title">
                    <FontAwesomeIcon icon={faBug} /> Debug Payload
                  </span>
          ...
          {searchReplacePrompts.length > 0 && (
            <div className="message search-replace-message">
              <div className="message-content">
                <div className="response-header">
                  <span className="response-title">
                    <FontAwesomeIcon icon={faSearch} /> Search & Replace Prompts
                  </span>
                  <div className="response-actions">
                    <button
                ...
                <div className="search-replace-content">
                  <h4>
                    Generated {searchReplacePrompts.length} search/replace
                    operation(s)
                  </h4>
                    ...
                    <div key={prompt.id || index} className="prompt-item">
                      <div className="prompt-header">
                        ...
                        <span className="prompt-confidence">
                          {Math.round((prompt.confidence || 0.8) * 100)}%
                        </span>
                      </div>
          ...
          {aiResponse && aiResponse.success && (
            <div className="message ai-message">
              <div className="message-content">
                <div className="response-header">
                  <span className="response-title">
                    <FontAwesomeIcon icon={faRobot} /> AI Response
                  </span>
                ...
                {aiResponse.edits.length > 0 ? (
                  ...
                  <div className="no-edits-message">
                    <FontAwesomeIcon icon={faComments} /> This is a
                    conversational response with no code changes to apply.
                  </div>
                )}
        ...
        {showConversationHistory && (
          <div className="conversation-history-panel">
            <div className="history-header">
              <h4>
                <FontAwesomeIcon icon={faComments} /> Conversation History
              </h4>
              <button
            ...
            <div className="conversation-list">
              {conversationStore
                ...
                .filter(
                  (conv) =>
                    conv.projectPath === projectContext?.currentProject?.path,
                )
                .map((conversation) => (
                  <div
                    key={conversation.id}
