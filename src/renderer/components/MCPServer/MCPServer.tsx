...
const MCPServer: React.FC<MCPServerProps> = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      // Cleanup listener if possible (though onAuthStateChanged might not return unsubscribe in some implementations, standard practice is to handle it)
  ...
  const handleConfigureClaudeDesktop = async (server: RunningMCPServer) => {
    try {
      ...
      const servicesList = [
        '• My DB - Query imported tables (Excel, CSV)',
    ...
    <div className="mcp-server">
      ...
      {showScriptEditor ? (
        ...
        <RunningServersTabs
          ...
          hasValidOAuthToken={hasValidOAuthToken}
