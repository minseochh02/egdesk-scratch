...
/** UI-only: website crawl vs. register by company name (persistence TBD) */
type CompanyLookupMode = 'website' | 'companyName';
...
const CompanyResearchTab: React.FC<CompanyResearchTabProps> = ({
  ...
  const [lookupMode, setLookupMode] = useState<CompanyLookupMode>('website');
  ...
  const handleExport = async () => {
    ...
    setIsResearching(true); // Re-use researching state for loading
    try {
...
Generated by EGDesk Company Research`;
  ...
  const handleRegisterCompanyByName = () => {
    const name = registeredCompanyName.trim();
    if (!name) return;
    // Persistence and downstream flows will be added later.
  };
  ...
  return (
    <div className="crt">
      ...
      <div className="crt-header">
        <div className="crt-input-mode-row" role="tablist" aria-label="How to look up the company">
          <button
            type="button"
            role="tab"
            aria-selected={lookupMode === 'website'}
            className={`crt-mode-tab${lookupMode === 'website' ? ' crt-mode-tab--active' : ''}`}
          ...
          <button
            type="button"
            role="tab"
            aria-selected={lookupMode === 'companyName'}
            className={`crt-mode-tab${lookupMode === 'companyName' ? ' crt-mode-tab--active' : ''}`}
            onClick={() => setLookupMode('companyName')}
          >
            Name only
          </button>
        </div>
            ...
            <>
              <div className="crt-input-wrap">
                ...
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' && registeredCompanyName.trim()) {
                      handleRegisterCompanyByName();
                    }
                  }}
              ...
              <button
                type="button"
                onClick={handleRegisterCompanyByName}
          ...
          <div className="crt-prompt crt-email-modal">
            ...
            <div className="crt-export-info">
              <div className="crt-export-files">
                <span className="crt-file-badge">📄 Combined Research Report</span>
              </div>
              <p className="crt-export-format">Will be sent as a singular DOCX file</p>
            </div>
                  ...
                  {resultTab === 'executive' && execSummary && (
                    <div className="crt-report">
                      <div className="crt-report-actions">
                        <button onClick={handleExport}>⬇️ Export Combined DOCX</button>
