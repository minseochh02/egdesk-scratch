import {
  ...
  SearchResult,
} from '../../AIEditor/services/codespaceVectorService';
import { ChatMessage } from '../types';
import { AIKey } from '../../AIKeysManager/types';
import {
  AISemanticKeywordService,
  SemanticKeyword,
} from '../../AIEditor/services/aiSemanticKeywordService';
...
export class CodespaceChatService {
  ...
  async enhanceMessageWithContext(
    ...
    try {
      ...
      if (aiKey && model) {
        ...
        try {
          projectTree = await this.buildFullProjectDirectoryTree(
            this.currentWorkspacePath,
          );
        } catch (error) {
          console.warn(
            'Failed to build full directory tree, falling back to cached files:',
            error,
          );
          ...
          const workspacePath =
            context?.workspacePath || this.currentWorkspacePath;

          const dirMap: Record<string, string[]> = {};
          for (const f of files) {
            const abs = f.path || '';
            const rel = abs.startsWith(workspacePath)
              ? abs.substring(workspacePath.length).replace(/^\/+/, '')
              : abs;
        ...
        try {
          ...
          keywordResponse = await keywordService.generateKeywords(
            aiKey,
            model,
            keywordRequest,
          );
          ...
          keywordResponse = { success: false, keywords: [] } as {
            success: boolean;
        ...
        if (keywordResponse?.success && keywordResponse.keywords.length > 0) {
            ...
            if (kw.category === 'primary') {
              try {
                ...
                if (res.success && typeof res.content === 'string') {
                  ...
                  const results = await this.codespaceService.searchCodespace(
                    kw.keyword,
                    1,
                  );
                  if (results.length > 0) {
                    primaryFiles.push({
                      path: results[0].file.path,
                      content: results[0].file.content,
                    });
                  }
                }
              } catch (_) {
          ...
          console.log(
            '🔍 Primary files loaded:',
            primaryFiles.length,
            primaryFiles.map((pf) => `${pf.path} (${pf.content.length} chars)`),
          );
          console.log(
            '🔍 Secondary/tech metadata:',
            secondaryTechMeta.length,
            secondaryTechMeta.map((m) => `${m.path} (${m.category})`),
          );
  ...
  createContextualSystemPrompt(
    ...
    contextPrompt +=
      'You have access to the following relevant project context.\n\n';

    // Include full contents for primary files (if any)
    if (
      codespaceContext.primaryFiles &&
      codespaceContext.primaryFiles.length > 0
    ) {
    ...
    if (
      codespaceContext.relevantFiles &&
      codespaceContext.relevantFiles.length > 0
    ) {
    ...
    if (
      codespaceContext.secondaryTechMeta &&
      codespaceContext.secondaryTechMeta.length > 0
    ) {
      ...
      for (const meta of codespaceContext.secondaryTechMeta) {
        ...
        if (meta.relatedTerms && meta.relatedTerms.length > 0)
          contextPrompt += ` (related: ${meta.relatedTerms.join(', ')})`;
        contextPrompt += '\n';
      }
      contextPrompt += '\n';
    }
    ...
    contextPrompt +=
      'Use the PRIMARY FILES as the main source of truth. Use SNIPPETS for supporting evidence, and consider SECONDARY/TECHNICAL paths as references. When referencing code, always mention the file path and provide specific, actionable suggestions.';
   ...
   */
  private async buildFullProjectDirectoryTree(
    rootPath: string,
  ): Promise<string> {
