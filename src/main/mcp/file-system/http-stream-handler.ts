/**
 * HTTP Streaming Handler for File System MCP Protocol
 * 
...
/**
 * HTTP Streaming Handler for File System MCP
 * 
 ...
 */
export class FileSystemHTTPStreamHandler {
  ...
  async handleStream(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
    console.log('📡 File System MCP - HTTP Streaming connection established');
    ...
    req.on('end', () => {
      console.log('📡 File System MCP - HTTP Streaming connection ended');
      res.end();
    });

    req.on('error', (error) => {
      console.error('File System MCP - HTTP Streaming connection error:', error);
      res.end();
    });

    // Handle client disconnect
    req.on('close', () => {
      console.log('📡 File System MCP - HTTP Streaming connection closed');
    });
  }
  ...
  private handleInitialize(id: string | number | null | undefined, params: any): JSONRPCResponse {
    console.log('🔄 Initializing File System MCP session (HTTP Stream)');
  ...
  private handlePing(id: string | number | null | undefined): JSONRPCResponse {
    console.log('🏓 PING received (File System MCP)');
  ...
  private handleToolsList(id: string | number | null | undefined): JSONRPCResponse {
    console.log('📋 Listing File System tools (HTTP Stream)');
  ...
  private async handleToolCall(
    ...
    console.log(`🔧 Calling File System tool: ${name} (HTTP Stream)`);
