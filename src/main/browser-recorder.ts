...
interface RecordedAction {
  ...
  // Accessibility attributes for better selector strategies
...
export class BrowserRecorder {
    ...
    await this.context.addInitScript(() => {
      ...
      const handleClick = (e: MouseEvent) => {
        ...
        const event: any = {
          ...
          innerText: capturedData.innerText,
          role: capturedData.role,
          ariaLabel: capturedData.ariaLabel
        };

        console.log('📤 Click event prepared - Selector:', selector, 'XPath:', xpath, 'Text:', event.text, 'Role:', event.role, 'AriaLabel:', event.ariaLabel);
  ...
  private async injectKeyboardListener(): Promise<void> {
    ...
    await this.page.exposeFunction('__playwrightRecorderOnClick', async (data: any) => {
      ...
      console.log('  - AriaLabel:', data.ariaLabel);
      console.log('  - InnerText:', data.innerText);
      ...
      switch (action.type) {
        ...
        case 'download':
          ...
          if (action.selector === 'download-complete') {
            ...
            lines.push(`      const finalPath = path.resolve(downloadsPath, suggestedFilename);`);
            lines.push(``);
            lines.push(`      // Ensure download directory exists`);
            lines.push(`      if (!fs.existsSync(downloadsPath)) {`);
            lines.push(`        fs.mkdirSync(downloadsPath, { recursive: true });`);
            lines.push(`      }`);
            lines.push(``);
            lines.push(`      // Wait for download to finish`);
            lines.push(`      await page.waitForTimeout(500);`);
            lines.push(``);
            lines.push(`      // Get the temporary file path (more reliable than saveAs on Windows)`);
            lines.push(`      const tempPath = await download.path();`);
            lines.push(``);
            lines.push(`      if (!tempPath) {`);
            lines.push(`        throw new Error('Download path is null - download may have failed');`);
            lines.push(`      }`);
            lines.push(``);
            lines.push(`      console.log('📥 Download temp path:', tempPath);`);
            lines.push(`      console.log('📂 Final destination:', finalPath);`);
            lines.push(``);
            ...
            lines.push(`      console.log('✅ Temp file exists, size:', tempStats.size, 'bytes');`);
            lines.push(``);
            lines.push(`      // Manual copy with retry logic (handles file locking on Windows)`);
            lines.push(`      let copySuccess = false;`);
            ...
            lines.push(`          console.log('✅ copyFileSync completed');`);
            lines.push(``);
            lines.push(`          // Verify the destination file exists and has content`);
            lines.push(`          if (fs.existsSync(finalPath)) {`);
            lines.push(`            const stats = fs.statSync(finalPath);`);
            lines.push(`            if (stats.size > 0) {`);
            lines.push(`              console.log('✅ Download completed:', finalPath);`);
            lines.push(`              console.log('📊 File size:', stats.size, 'bytes');`);
            lines.push(`              copySuccess = true;`);
            lines.push(`              break;`);
            lines.push(`            } else {`);
            lines.push(`              throw new Error('Destination file is empty (0 bytes)');`);
            lines.push(`            }`);
            lines.push(`          } else {`);
            lines.push(`            throw new Error('Destination file not found after copy');`);
            lines.push(`          }`);
    ...
    for (let i = 0; i < partialActions.length; i++) {
      ...
      switch (action.type) {
        ...
        case 'download':
          ...
          lines.push(``);
          lines.push(`      const tempPath = await download.path();`);
          lines.push(`      if (!tempPath) throw new Error('Download path is null');`);
          lines.push(``);
          lines.push(`      console.log('✓ Temp file:', tempPath);`);
          lines.push(`      if (!fs.existsSync(tempPath)) throw new Error('Temp file not found');`);
          lines.push(``);
          lines.push(`      // Copy with retry logic`);
          lines.push(`      let copySuccess = false;`);
          lines.push(`      for (let attempt = 1; attempt <= 3; attempt++) {`);
          lines.push(`        try {`);
          lines.push(`          fs.copyFileSync(tempPath, finalPath);`);
          lines.push(`          if (fs.existsSync(finalPath) && fs.statSync(finalPath).size > 0) {`);
          lines.push(`            console.log('✓ Downloaded:', suggestedFilename, 'to', finalPath);`);
          lines.push(`            copySuccess = true;`);
          lines.push(`            break;`);
          lines.push(`          }`);
          lines.push(`        } catch (err) {`);
          lines.push(`          if (attempt < 3) await page.waitForTimeout(1000);`);
          lines.push(`        }`);
          lines.push(`      }`);
          lines.push(`      if (!copySuccess) throw new Error('Download copy failed');`);
          lines.push(`    }`);
          break;
