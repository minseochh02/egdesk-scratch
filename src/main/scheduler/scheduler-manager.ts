...
export class SchedulerManager {
  ...
  private async executeTask(task: ScheduledTask): Promise<void> {
    ...
    try {
      ...
      // Handle special ELECTRON_SCRIPT commands
      if (command.startsWith('ELECTRON_SCRIPT:')) {
        const scriptPath = command.replace('ELECTRON_SCRIPT:', '');
        command = `node "${scriptPath}" "${task.id}"`;
        console.log(`📝 Step 1: Running script ${scriptPath}`);
      } else {
        console.log(`📝 Step 1: Running command: ${command}`);
      }
