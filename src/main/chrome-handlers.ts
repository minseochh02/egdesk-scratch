...
// This test includes actual timing delays captured during recording`
  ...
  // Unified browser recorder replay entrypoint (action engine only)
  ipcMain.handle(
    'run-browser-recording-replay',
    async (
      event,
      { testFile, replayParams }: { testFile: string; replayParams?: BrowserRecordingReplayParams }
    ) => {
      try {
        if (!fs.existsSync(testFile)) {
          ...
          return { success: false, error: `Test file not found: ${testFile}` };
        }

        const normalizedReplayParams: BrowserRecordingReplayParams = replayParams ?? {};
        const ar = await runBrowserRecordingActionReplay(testFile, normalizedReplayParams, event);
        return ar.success
        ...
        return {
          success: false,
          error: error?.message || 'Failed to run browser recording replay',
        };
      }
    }
  );
  ...
  ipcMain.handle('run-chain', async (event, { chainId }) => {
    try {
      ...
      for (const script of sortedScripts) {
        ...
        // Run the script using the legacy chain execution logic
        ...
        // Extract test body (same logic as legacy chain execution)
        let testBody = '';
