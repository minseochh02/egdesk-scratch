...
const electronHandler = {
  ...
  debug: {
    ...
    exportSingleTest: (testPath: string) => ipcRenderer.invoke('browser-recorder:export-single', { testPath }),
    exportAllTests: () => ipcRenderer.invoke('browser-recorder:export-all'),
    importTests: () => ipcRenderer.invoke('browser-recorder:import-tests'),
  ...
  financeHub: {
    ...
    resolveCertificateIndex: (metadata: { name: string; issuer: string; notAfter?: string; folder?: string; sortByExpiry?: boolean }) =>
    ...
    /** IBK B2B → 전자어음 → 조회 → 배서내역조회. Max 12-month span; auto-imports after download. */
    syncIbkEndorsements: (opts?: { startDate?: string; endDate?: string }) =>
      ipcRenderer.invoke('finance-hub:sync-ibk-endorsements', opts || {}),
  ...
  financeHubDb: {
    ...
    updateAccountMetadata: (bankId: string, accountNumber: string, metadata: any) =>
      ipcRenderer.invoke('sqlite-financehub-update-account-metadata', bankId, accountNumber, metadata),
