...
import { initializeNeuronSchema } from './ai-center-init';
...
export function getDatabaseSize(dbPath: string): number {
  ...
  export function getNeuronDatabasePath(): string {
    return path.join(app.getPath('userData'), 'database', 'neuron.db');
  }

  export function getNeuronDatabase(): Database.Database {
    const neuronDbPath = getNeuronDatabasePath();
    return new Database(neuronDbPath);
  }
...
export interface DatabaseInitResult {
  ...
  syncDbPath?: string;
  neuronDatabase?: Database.Database;
  neuronDbPath?: string;
}
...
export async function initializeSQLiteDatabase(): Promise<DatabaseInitResult> {
  try {
    ...
    const neuronDbPath = path.join(dataDir, 'ai-system.db');
    ...
    console.log('  Neuron DB:', neuronDbPath);
    ...
    // DB 실시간 데이터 변이 감지 및 디스패처 기동
    try {
      const { DBChangeDetector } = require('./db-change-detector');
      DBChangeDetector.getInstance().watchNeuron(neuronDb);
      DBChangeDetector.getInstance().watchFinanceHub(financeHubDb);
      console.log('🔔 DB Real-time Change Detectors active for AI Center & FinanceHub');
    } catch (detectErr) {
      console.error('⚠️ Failed to launch DB Change Detectors:', detectErr);
    }
