// ============================================
// Migration 020: Add 'replace-all' to sync_configurations duplicate_action CHECK constraint
// ============================================
// Updates the CHECK constraint to include the new 'replace-all' duplicate action option

import Database from 'better-sqlite3';

export function migrate020AddReplaceAllMode(db: Database.Database): void {
  console.log('🔄 Migration 020: Adding replace-all to sync_configurations duplicate_action...');
  ...
  if (tableInfo.sql.includes("'replace-all'")) {
    console.log('  ✅ sync_configurations already has replace-all in CHECK constraint - skipping');
    return;
  }
  ...
  db.exec(`
    ...
    CREATE TABLE sync_configurations_new (
      ...
      source TEXT DEFAULT 'browser' CHECK(source IN ('browser', 'desktop')),
  ...
  console.log('  ✅ Migration 020 complete - sync_configurations now supports replace-all');
}
