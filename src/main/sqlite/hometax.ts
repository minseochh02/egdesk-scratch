...
import { TaxInvoiceData, TaxExemptInvoiceData, CashReceiptData } from '../hometax-excel-parser';
...
/**
 * Import tax-exempt invoices into database
 */
export function importTaxExemptInvoices(
  db: Database.Database,
  businessNumber: string,
  invoiceType: 'sales' | 'purchase',
  ...
  try {
    console.log(`[Hometax DB] Importing ${invoices.length} ${invoiceType} tax-exempt invoices for ${businessNumber}`);

    const insertStmt = db.prepare(`
      INSERT INTO tax_exempt_invoices (
        business_number, invoice_type,
        작성일자, 승인번호, 발급일자, 전송일자,
        공급자사업자등록번호, 공급자종사업장번호, 공급자상호, 공급자대표자명, 공급자주소,
        공급받는자사업자등록번호, 공급받는자종사업장번호, 공급받는자상호, 공급받는자대표자명, 공급받는자주소,
    ...
    const transaction = db.transaction((rows: TaxExemptInvoiceData[]) => {
    ...
    transaction(invoices);
    return { success: true, inserted, duplicate };
  } catch (error) {
    console.error('[Hometax DB] Error importing tax-exempt invoices:', error);
    return {
      success: false,
      inserted: 0,
      duplicate: 0,
...
/**
 * Get tax-exempt invoices with filters
 */
export function getTaxExemptInvoices(
  db: Database.Database,
  filters: {
    businessNumber?: string;
    invoiceType?: 'sales' | 'purchase';
  ...
  try {
    ...
    console.error('[Hometax DB] Error getting tax-exempt invoices:', error);
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error'
    };
 ...
 */
export function saveTaxExemptSpreadsheetUrl(
  db: Database.Database,
  businessNumber: string,
  invoiceType: 'sales' | 'purchase',
  spreadsheetUrl: string
  ...
  try {
    const columnName =
      invoiceType === 'sales'
        ? 'tax_exempt_sales_spreadsheet_url'
        : 'tax_exempt_purchase_spreadsheet_url';
    ...
    console.error('[Hometax DB] Error saving tax-exempt spreadsheet URL:', error);
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error'
    };
 ...
 */
export function getTaxExemptSpreadsheetUrl(
  db: Database.Database,
  businessNumber: string,
  invoiceType: 'sales' | 'purchase'
): { success: boolean; spreadsheetUrl?: string; error?: string } {
  try {
    ...
    console.error('[Hometax DB] Error getting tax-exempt spreadsheet URL:', error);
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error'
    };
