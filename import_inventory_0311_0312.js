...
console.log('Excel Inventory Data Import Script - March 11-12, 2026');
console.log('='.repeat(80));
...
// Specific files to import
const targetFiles = ['inventory0311.xlsx', 'inventory0312.xlsx'];
  ...
  try {
    ...
    for (const cell of row0) {
      if (cell && typeof cell === 'string') {
        // Match pattern: "회사명 : (주)영일오엔씨 / 2026/03/11" or similar
...
// Print summary
console.log('='.repeat(80));
console.log('IMPORT SUMMARY - March 11-12, 2026');
console.log('='.repeat(80));
console.log(`Files Processed: ${totalFiles}/${targetFiles.length}`);
...
const statsNew = dbVerify.prepare(`
  SELECT
    imported_at,
    COUNT(*) as row_count
  FROM inventory
  WHERE imported_at IN ('2026-03-11', '2026-03-12')
...
console.log('='.repeat(80));
console.log('Newly Imported Data (March 11-12):');
for (const stat of statsNew) {
  console.log(`  ${stat.imported_at}: ${stat.row_count} rows`);
}
console.log();
console.log('March 2026 Total:');
