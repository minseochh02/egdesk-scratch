...
console.log('Excel Inventory Data Import Script');
console.log('='.repeat(80));
  ...
  try {
    ...
    for (const cell of row0) {
      if (cell && typeof cell === 'string') {
        // Match pattern: "회사명 : (주)영일오엔씨 / 2026/01/01"
    ...
    if (!importDate) {
      ...
      // Try to extract date from filename (format: 창고별재고YYYYMMDD.xlsx)
...
// Update the user_tables metadata with new row count
try {
  const newRowCount = db.prepare('SELECT COUNT(*) as count FROM inventory').get().count;
  db.prepare(`
    UPDATE user_tables
    SET rowCount = ?, updatedAt = datetime('now')
    WHERE tableName = 'inventory'
  `).run(newRowCount);
} catch (error) {
  console.log(`Warning: Could not update user_tables metadata: ${error.message}`);
}
...
console.log('='.repeat(80));
console.log(`Total rows in inventory table: ${stats.total_rows}`);
console.log(`Unique import dates: ${stats.unique_dates}`);
